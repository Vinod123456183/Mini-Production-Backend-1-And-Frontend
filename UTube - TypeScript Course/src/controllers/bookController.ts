...
export const addBook = async (req: Request, res: Response) => {
    ...
    if (![ROLES.creator, ROLES.admin].includes(req.role as string)) {
        return res.status(401).json({
            success: false, message: "You are not allowed to access the resource",
        } as IResponse)
    }
    ...
    try {
        ...
        return res.status(201).json({ success: true, message: "Book added", data: book } as IResponse)
...
// http://localhost:5000/api/book/updateBook/30747391743
